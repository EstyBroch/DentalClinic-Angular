...
export class NewPatientFormComponent implements OnInit {
  ...
  constructor(private emp_servise: EmployeesService,private router:Router) { }

  ngOnInit(): void {
    if (this.router.url.includes('secretary')) {
  ...
  newPatientForm: FormGroup = new FormGroup(
    {
      ...
      "identity":new FormControl(null,[Validators.required,Validators.maxLength(9),Validators.minLength(9)]),
